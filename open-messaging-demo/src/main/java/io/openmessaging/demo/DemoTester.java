...
//                    Assert.assertEquals(messagesForTopic1.get(topic1Offset++), message);
                    Assert.assertArrayEquals(((BytesMessage)messagesForTopic1.get(topic1Offset++)).getBody(), ((BytesMessage)message).getBody());
                } else {
                    Assert.assertEquals(queue1, queue);
                    Assert.assertArrayEquals(((BytesMessage)messagesForQueue1.get(queue1Offset++)).getBody(), ((BytesMessage)message).getBody());
                }
            }
            ...
            while (true) {
                ...
                if (topic != null) {
                    if (topic.equals(topic1)) {
                        ...
                        Assert.assertArrayEquals(((BytesMessage)messagesForTopic2.get(topic2Offset++)).getBody(), ((BytesMessage)message).getBody());
                    }
                } else {
                    Assert.assertEquals(queue2, queue);
                    Assert.assertArrayEquals(((BytesMessage)messagesForQueue2.get(queue2Offset++)).getBody(), ((BytesMessage)message).getBody());
                }
            }
            long endConsumer = System.currentTimeMillis();
            long T2 = endConsumer - startConsumer;
